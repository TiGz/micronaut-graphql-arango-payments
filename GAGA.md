# Going GAGA for RAD 
Introducing a new extensible architecture for rapid application development that can scale up and out from initial monolith to multi-team micro-services and presents 2 separate GraphQL Schemas - one for querying and another for making updates (mutations).
...
The main advantages of a GraphQL combine together to make client application development much faster:
* The schema defines the interface between client and server
* The schema is fully and interactively explorable via the GraphIQL explorer/console
* Each client is in full control of the data they request which makes for a faster UI (no over/under fetching problem)

### How GAGA handles the GraphQL query performance problem
Usually the main problem with building a friendly GraphQL API is that it can be easy to build a schema that allows clients to ask for lots of data that involves lots of leaf fetches which usually each require a database round-trip and this can really hurt performance and scalability.
...
Another great feature of Micronaut is how easiy it is it wire up annotation driven aspects. Here is an example of adding ArangoDB transaction management via an annotation:
...
Which allows us to handle mutations by both updating the database and adding to a mutation_log collection which can then be used for an [outbox pattern event emitter](https://microservices.io/patterns/data/transactional-outbox.html) so that those mutations can be picked up and acted upon asynchronously:
...
The service method will write to both the **payment_request** collection and then the **mutation_log** collection but it will do it atomically in a transaction and will rollback if either is unsuccessful.